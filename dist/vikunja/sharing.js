...
export const toolDefinitions = [
    {
        ...
        description: 'List the users a project is directly shared with, and their permission level.',
        inputSchema: {
            type: 'object',
    ...
    },
    {
        ...
        description: 'Share a project with a user by USERNAME. Resolve the username via search_users if needed.',
        inputSchema: {
            type: 'object',
            properties: {
                ...projectIdProp,
    ...
    },
    {
        ...
        description: 'List the teams a project is shared with, and their permission level.',
        inputSchema: {
            type: 'object',
            properties: { ...projectIdProp },
            required: ['projectId'],
    ...
    },
    {
        ...
        description: 'List the public link shares for a project (shareable URLs), with their permission level and sharing type.',
        inputSchema: {
            type: 'object',
            properties: { ...projectIdProp },
            required: ['projectId'],
    ...
    },
    {
        ...
        description: 'Create a public link share for a project. sharing_type: 1 = no password, 2 = password-protected (then provide password).',
        inputSchema: {
            type: 'object',
            properties: {
                ...projectIdProp,
...
export const handlers = {
    ...
    add_project_user: async (request) => {
        ...
        const r = await addProjectUser(projectId, username, permission);
        return isErr(r)
    ...
    add_project_team: async (request) => {
        ...
        if (!validPermission(permission))
            return err('permission must be 0, 1, or 2');
        const r = await addProjectTeam(projectId, teamId, permission);
        return isErr(r)
    ...
    create_project_share: async (request) => {
        const { projectId, permission, sharing_type, password, name } = request.params.arguments || {};
        if (!num(projectId))
            return err('Invalid project ID');
        if (!validPermission(permission))
            return err('permission must be 0, 1, or 2');
