...
export type SavedFilter = SavedFilterInput & {
  id: number;
...
const projectIdToFilterId = (projectId: number): number | null =>
...
const listSavedFilters = async () => {
  ...
  const filters = (response.data ?? [])
    ...
    .filter((p): p is typeof p & { filter_id: number } => p.filter_id !== null);
  return { isError: false as const, data: filters };
};

const getSavedFilter = async (filterId: number) =>
...
const updateSavedFilter = async (
  ...
  wrapRequest(serviceInstance.post<SavedFilter>(`/filters/${filterId}`, data));
...
const FILTER_SHAPE_DESCRIPTION =
  'Object containing query params: s (search), filter (DSL string, e.g. "done = false"), sort_by (array of field names), order_by (array of "asc"/"desc"), filter_include_nulls, filter_timezone.';

export const toolDefinitions = [
  {
    name: 'list_saved_filters',
    description:
      "List the user's saved filters. Vikunja exposes these as virtual projects with negative IDs; this tool unwraps them and returns the underlying filter_id you can pass to get_saved_filter.",
    ...
    inputSchema: {
      ...
      properties: {
        filterId: {
          type: 'integer',
          description:
            'The filter ID (positive integer, NOT the negative virtual-project ID)',
        },
      },
      required: ['filterId'],
    },
  },
  {
    ...
    description:
      'Create a saved filter. Once created, tasks matching it appear under a virtual project with id -(filter_id + 1).',
    inputSchema: {
      type: 'object',
      properties: {
        title: { type: 'string' },
