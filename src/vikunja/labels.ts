...
export const toolDefinitions = [
  {
    name: 'list_labels',
    description:
      'List labels available to the current user. Use this to discover label IDs before attaching them to a task.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        hex_color: {
          type: 'string',
          description:
            'Hex color WITHOUT leading # (Vikunja convention, e.g. "dc2626"). Schema permits "#RRGGBB" too.',
        },
      },
      required: ['title'],
    },
  ...
  },
  {
    name: 'delete_label',
    description:
      'Delete a label by ID. Removes it from all tasks. Cannot be undone.',
    inputSchema: {
      type: 'object',
      properties: {
        labelId: { type: 'integer', description: 'The ID of the label' },
