...
const searchUsers = async (search: string) =>
  wrapRequest(
    serviceInstance.get<Array<User> | null>('/users', {
      params: { s: search },
    }),
  );

const getCurrentUser = async () =>
  wrapRequest(serviceInstance.get<User>('/user'));
...
export const toolDefinitions = [
  {
    name: 'search_users',
    description:
      'Search users by exact username (Vikunja does NOT do partial-match here, and email lookup does not work). Use to resolve a username to a user ID for assignees. Returns null if no exact match.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Get the user who owns the API token (useful for self-assignment).',
