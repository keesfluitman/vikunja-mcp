...
const removeTeamMember = async (teamId: number, username: string) =>
  wrapRequest(serviceInstance.delete(`/teams/${teamId}/members/${username}`));
...
export const toolDefinitions = [
  {
    name: 'list_teams',
    description:
      'List teams the current user belongs to. Use this to discover team IDs before sharing a project with a team or managing members.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'update_team',
    description:
      'Update a team’s name, description, or visibility. Fetches the team first and merges, so omitted fields are preserved.',
    inputSchema: {
      type: 'object',
      properties: {
        teamId: { type: 'integer', description: 'The ID of the team' },
  ...
  },
  {
    name: 'delete_team',
    description:
      'Delete a team by ID. Removes its access to all shared projects. Cannot be undone.',
    inputSchema: {
      type: 'object',
      properties: {
        teamId: { type: 'integer', description: 'The ID of the team' },
  ...
  },
  {
    ...
    description:
      'Add a user to a team by USERNAME (not user ID). Resolve the username via search_users if needed.',
    inputSchema: {
      type: 'object',
      properties: {
        teamId: { type: 'integer', description: 'The ID of the team' },
  ...
  },
  {
    ...
    description:
      'Remove a user from a team by USERNAME (the remove endpoint is keyed by username, not user ID).',
    inputSchema: {
      type: 'object',
      properties: {
        teamId: { type: 'integer', description: 'The ID of the team' },
...
export const handlers: Record<string, ToolHandler> = {
  list_teams: async request => {
    ...
    const perPage =
      typeof args.per_page === 'number' ? Math.min(args.per_page, 50) : 50;
